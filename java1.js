...
const jsonData = {
    ...
    "data": [
        ...
        { "id": 5, "year": 2004, "color": "#E2583E", "pantone_value": "17-1456" },
        { "id": 6, "name": "blue turquoise", "year": 2005, "color": "#53B0AE", "pantone_value": "15-5217" }
    ]
};

// 📌 JSON Verisini Konsola Yazdıran Fonksiyon
function printJsonData(data) {
    console.log("📌 JSON Renk Listesi:");
    console.log("--------------------------------");

    data.data.forEach(color => {
        console.log(`ID: ${color.id}`);
        console.log(`İsim: ${color.name || "Bilinmiyor"}`);
        console.log(`Yıl: ${color.year}`);
        console.log(`HEX Kodu: ${color.color}`);
        console.log(`Pantone: ${color.pantone_value}`);
        console.log("--------------------------------");
    });
}
